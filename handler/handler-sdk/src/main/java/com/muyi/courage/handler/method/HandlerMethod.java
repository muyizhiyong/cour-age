...
public class HandlerMethod {
	...
	protected static Object findProvidedArgument(MethodParameter parameter, @Nullable Object... providedArgs) {
		if (!ObjectUtils.isEmpty(providedArgs)) {
			for (Object providedArg : providedArgs) {
				// 确定指定的对象是否与该类表示的对象分配兼容。该方法是Java语言instanceof操作符的动态等效。
				// 如果指定的对象参数是非null的，并且可以转换为该类对象所表示的引用类型，而不会引发ClassCastException，
	 ...
	 */
	@Nullable
	protected static Object findProvidedArgument(MethodParameter parameter, @Nullable Object providedArg) {
		if (!ObjectUtils.isEmpty(providedArg)) {
			// 确定指定的对象是否与该类表示的对象分配兼容。该方法是Java语言instanceof操作符的动态等效。
			// 如果指定的对象参数是非null的，并且可以转换为该类对象所表示的引用类型，而不会引发ClassCastException，
			// 则该方法返回true。否则返回false。
			if (parameter.getParameterType().isInstance(providedArg)) {
