...
public class SocketUtil {
	...
	/**
	 * 从socket通道上读取数据（适用于对接上海银行生物认证平台）
	 * @param in
	 * @return
	 * @throws IOException
	 * @throws ZipException
	 */
	public static byte[] readSocketSHAuth(DataInputStream in)throws IOException, ZipException {
		if (in == null){
			throw new IllegalArgumentException("readSocket输入参数为空");
		}
		byte[] retData = new byte[1024*4];
		String retDataStr = "";
		while ((in.read(retData) != -1)){
			retDataStr = retDataStr + new String(retData);
		}
		return retDataStr.getBytes();
	}
	...
	/**
	 * 发送报文（适用于对接上海银行生物认证平台）
	 *
	 * @param out
	 * @param sendBody xml格式报文
	 * @param packetLen
	 * @param retMsg
	 * @return
	 */
	public static Boolean sendMessageSHAuth(OutputStream out,byte[] sendBody, int packetLen, StringBuffer retMsg){

		byte[] b = sendBody;
		int sendTimes = (b.length / packetLen);// 发包次数
		if (b.length % packetLen > 0) {
			sendTimes++;
		}
		String test = "";
		try {
			...
			for (int i = 1; i <= sendTimes; i++) {
				...
				System.arraycopy(b, startLen, byteSend, 0, zipLen);
				out.write(byteSend);
				out.flush();
				startLen = endLen;
			}
